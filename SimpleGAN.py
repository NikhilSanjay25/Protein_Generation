...
class Critic(nn.Module):
    def __init__(self):
        ...
        # Nodes: MAX_ATOMS * NUM_ATOM_TYPES
        # Adj: MAX_ATOMS * MAX_ATOMS * NUM_BOND_TYPES
        self.input_dim = (Config.MAX_ATOMS * Config.NUM_ATOM_TYPES) + \
                         (Config.MAX_ATOMS * Config.MAX_ATOMS * Config.NUM_BOND_TYPES)
        ...
        self.net = nn.Sequential(
            ...
            nn.Linear(64, 1) # Output scalar score (Wasserstein distance)
        )

    def forward(self, x, adj):
        # Flatten inputs
        B = x.shape[0]
        x_flat = x.view(B, -1)       # Flatten nodes
        adj_flat = adj.view(B, -1)   # Flatten adjacency
        
        # Concatenate node features and bond features
        combined = torch.cat([x_flat, adj_flat], dim=1)
...
# --- Configuration ---
class Config:
    MODEL_DIR = 'models_var2_simple_mlp'
# ---
